...
// line-scan parser has no package view. When the only Go "build failures"
// are legacy "[build failed]" output lines yet the runner exited 0, the exit
// status vetoes the untrusted output (the same principle as ErrWaitDelay).
