...
| W-02 | `notify_on_complete` is read: `emitRequirementOutcome` now sends a webhook notification for `REQ_COMPLETED` (info) / `REQ_BLOCKED` (error, pointing at `.vxd-fix-gaps.md`), synchronously with a 10 s bound because it is the terminal event of the run. Gated by the new `notify.FilteredNotifier` allowlist. `TestAudit_NotifyOnCompleteUnwired` became `TestAudit_NotifyOnCompleteWired`. |
