...
| `security` | Security agent: per-story pre-merge security gate (scanners + LLM threat-model review against a growable OWASP/CWE knowledge base). `disable_gate` (turn the gate off), `gate_severity` (block threshold critical/high/medium/low), `auto_learn` (grow the KB from confirmed findings), `kb_path` (KB location). Standalone scans via `vxd security scan`. | `disable_gate: false`, `gate_severity: high`, `auto_learn: true`, `kb_path: <state_dir>/security/knowledge.json` |
