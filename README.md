...
| `routing` | Story complexity thresholds per tier, max retries before escalation, max concurrent agents, and optional history-aware tier selection (`adaptive`: demote to the cheapest tier with ≥80% success at that complexity or promote one tier when the default tier succeeds <40%, bounded at senior; overrides logged and stamped on STORY_STARTED as `adaptive_decision`) | `junior_max_complexity: 3`, `intermediate_max_complexity: 5`, `max_retries_before_escalation: 2`, `max_concurrent_agents: 5`, `adaptive: false`, `adaptive_min_samples: 5` |
| `planning` | Max story complexity (Fibonacci cap), sequential-file patterns, design approach, godmode flag, and `emit_scribe_story` (append a final README-scribe story per requirement that documents what was built and links the other docs — greenfield-aware, edits only within `<!-- vxd:scribe:start/end -->` markers on existing READMEs) | `max_story_complexity: 5`, `design_approach: ddd-tdd`, `godmode: false`, `emit_scribe_story: true` |
| `monitor` | Supervisor polling interval, stuck-agent threshold, context-freshness token budget, and `pipeline_timeout_s` (per-story deadline for the post-execution review+QA+merge pipeline; raise it for slow LLM reviewers like Codex or heavy conflict resolution) | `poll_interval_ms: 10000`, `stuck_threshold_s: 600`, `context_freshness_tokens: 150000`, `pipeline_timeout_s: 900` |
| `cleanup` | Worktree pruning strategy (`immediate`/`deferred`), branch retention window, log archive mode, and `delete_dangling_branches` (on requirement completion, delete the local + remote branches of stories that never merged — which auto-closes their open PRs, leaving no dangling branches/PRs) | `worktree_prune: immediate`, `branch_retention_days: 7`, `log_archive: dolt`, `delete_dangling_branches: true` |
...
| `billing` | Hourly consulting rate, currency, Fibonacci-to-hours range mapping, LLM cost accounting mode, and `max_usd_per_req` — a hard per-requirement spend cap computed from STORY_COST_RECORDED accounting (tokens + est USD by stage, surfaced in `vxd metrics`). Over cap the monitor emits `REQ_BUDGET_EXCEEDED` and cleanly pauses the requirement (no escalation-tier burn); raise the cap and run `vxd resume` to continue | `default_rate: 150.0`, `currency: USD`, `llm_costs.mode: subscription`, `max_usd_per_req: 0` (unlimited) |
| `qa` | Declarative success criteria evaluated after each story (output_contains, file_exists, file_contains, exit_code_zero, etc.); `disable_pre_merge_verify` (turn off the per-story pre-merge build/test gate); the requirement-completion gate — `disable_completion_gate` (turn off) + `completion_fix_cycles` (auto-fix attempts against a red composed mainline before blocking; `0`→default 2, negative→hard gate); and `flaky_retries` — re-run a FAILED **test** step up to N times before calling the failure real (lint/build are deterministic and never retried). A test that passes on retry keeps QA green but emits `STORY_QA_FLAKY` (step, attempts), counted per requirement in `vxd metrics`, so one flaky test can't silently burn escalation tiers. | No criteria by default; standard lint/build/test always run; `disable_pre_merge_verify: false`, `disable_completion_gate: false`, `completion_fix_cycles: 2`, `flaky_retries: 1` (`0` disables retrying) |
