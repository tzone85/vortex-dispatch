...
VXD already isolates files via git worktrees. It does not isolate databases. Stories touching client Postgres (mukuru-api, cashtask) share one dev DB → "agent stepped on another agent's work" failures, lost work on bad migrations, no safe place to test destructive SQL.
...
DB names follow: `vxd-<project>-<story-id>` where `<story-id>` is the full VXD story identifier (existing format: 8 chars of reqID + `-` + LLM-ID, e.g. `a8cbef1f-3a`). Example: `vxd-mukuru-api-a8cbef1f-3a`. NXD uses prefix `nxd-`. Enables provider-side `list-by-prefix` for orphan recovery.
