...
function renderAgents(agents) {
    ...
    if (!agents.length) {
        tbody.innerHTML = '<tr><td colspan="7" class="muted">No agents running</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = agents.map(a => {
        ...
        return '<tr>'
            ...
            + '<td>' + esc(a.type) + '</td>'
            + '<td>' + esc(a.model) + '</td>'
            + '<td class="' + statusClass + '">' + esc(a.status) + '</td>'
            + '<td>' + storyId + '</td>'
            + '<td>' + session + '</td>'
            + '<td><button class="btn-danger"'
            + ' onclick="confirmAction(\'Kill agent ' + agentId + '?\','
            + ' function(){ sendCommand(\'kill_agent\', {agent_id:\'' + agentId + '\'}); })">'
            + '&#x2715;</button></td>'
            + '</tr>';
    }).join('');
}
...
function renderStories(stories) {
    ...
    if (!sorted.length) {
        tbody.innerHTML = '<tr><td colspan="6" class="muted">No stories \u2014 run \'vxd plan\' to create a requirement</td></tr>';
        return;
    }
    // All values routed through esc().
    tbody.innerHTML = sorted.map(s => {
        const statusClass = 'status-' + esc(s.status);
        const statusText  = esc(s.escalation_tier > 0
        ...
        return '<tr>'
            ...
            + '<td>' + (s.complexity != null ? esc(String(s.complexity)) : '') + '</td>'
            + '<td>' + (s.escalation_tier || 0) + '</td>'
            + '<td>' + esc(s.title) + '</td>'
            + '<td>'
            + '<button class="btn-action" title="Retry"'
            + ' onclick="sendCommand(\'retry_story\', {story_id:\'' + storyId + '\'})">'
            + '&#x21BB;</button>'
            + '<button class="btn-action" title="Escalate"'
            + ' onclick="sendCommand(\'escalate_story\', {story_id:\'' + storyId + '\'})">'
            + '&#x2191;</button>'
            + '<button class="btn-action" title="Reassign"'
            + ' onclick="confirmAction(\'Reassign ' + storyId + '?\','
            + ' function(){ sendCommand(\'reassign_story\', {story_id:\'' + storyId + '\', target_tier:0}); })">'
            + '&#x21C4;</button>'
            + '</td>'
            + '</tr>';
    }).join('');
...
function buildEventRowInner(e) {
    return '<span class="event-time">'   + esc(e.timestamp)         + '</span>'
         + '<span class="' + eventClass(e.type) + '">' + esc(e.type) + '</span>'
         + '<span>' + esc(e.agent_id || '\u2014') + '</span>'
         + '<span>' + esc(e.story_id  || '\u2014') + '</span>';
}
...
function renderEvents(events) {
    ...
    log.innerHTML = filtered.map(e =>
        '<div class="event-row">' + buildEventRowInner(e) + '</div>'
    ).join('');
...
function appendEvent(e) {
    const log = document.getElementById('activity-log');
    if (activeFilter !== 'all' && !(e.type && e.type.startsWith(activeFilter))) return;
...
function renderEscalations(escalations) {
    ...
    container.innerHTML = escalations.map(e => {
        ...
        return '<div class="escalation-row">'
            + '<span>' + esc(e.story_id)   + '</span>'
            + '<span>' + esc(e.from_agent) + '</span>'
            + '<span>T' + esc(String(e.from_tier)) + '&#x2192;' + esc(String(e.to_tier)) + '</span>'
            + '<span class="' + cls + '">' + esc(e.status) + '</span>'
            + '<span class="muted">' + esc(e.reason) + '</span>'
            + '</div>';
    }).join('');
}
...
function sendCommand(action, payload) {
    ...
    ws.send(JSON.stringify({ type: 'command', action, payload: JSON.stringify(payload) }));
}
