...
| Requirement `blocked` (completion gate red) | Read `.vxd-fix-gaps.md` in the project directory, fix the gaps, commit and push them, then `vxd resume <req>` re-runs the gate. Resume refuses while the working tree is dirty: the gate verifies the checkout, and the fix agent it starts on red commits and pushes what it finds there |
